...
TIER_COLOR = {"Hot": "#ff4b4b", "Warm": "#ffa500", "Cold": "#4b8bff"}
SOURCE_ICON = {
    'gmail': '📩', 'webhook': '🔗', 'tally': '📋', 'typeform': '📝',
    'calendly': '📅', 'bulk_paste': '📂', 'csv': '📂', 'manual': '✍️', 'inbound': '📥',
    'outbound': '📣', 'event': '🎤', 'referral': '🤝'
}
...
def bulk_upsert(df, source='bulk'):
    ...
    for _, row in df.iterrows():
        lead_id, dup = upsert_lead({
            'lead_text': row.get('lead_text',''), 'company_name': row.get('company_name',''),
            'contact_name': row.get('contact_name',''), 'email': row.get('email',''),
            'website': row.get('website',''), 'source': source,
...
def save_analysis(lead_id, score, tier, fit_score, intent_score, reason, next_step, analysis):
    ...
    with get_connection() as conn:
        conn.execute("""INSERT INTO analyses(lead_id,score,tier,fit_score,intent_score,reason,next_step,analysis) VALUES(?,?,?,?,?,?,?,?)""",
...
def extract_company_size(text):
    ...
    if 'startup' in t: return (1, 10)
    if 'small' in t: return (1, 49)
    if 'mid' in t or 'growing' in t: return (50, 199)
    if 'enterprise' in t: return (200, 1000000)
    return None
...
def send_slack(lead, score, tier):
    ...
    msg = SLACK_TEMPLATES.get(tier, SLACK_TEMPLATES['Cold']).format(
        company_name=lead['company_name'] or '—', contact_name=lead['contact_name'] or '—',
        email=lead['email'] or '—', score=score)
    try:
...
def send_to_crm(lead, score, tier, enrichment):
    ...
    payload = {'lead_id': lead['id'], 'company_name': lead['company_name'], 'contact_name': lead['contact_name'],
               'email': lead['email'], 'website': lead['website'], 'score': score, 'tier': tier, 'enrichment': enrichment}
    try:
...
def simulate_webhook(payload_text):
    try:
        ...
        if isinstance(data, dict):
            return upsert_lead({'lead_text': data.get('lead_text',''), 'company_name': data.get('company_name',''),
                'contact_name': data.get('contact_name',''), 'email': data.get('email',''),
                'website': data.get('website',''), 'source': data.get('source','webhook'), 'raw_payload': payload_text})
    except Exception:
        pass
    return None, False
...
.lead-hot { border-left: 4px solid #ff4b4b; }
.lead-warm { border-left: 4px solid #ffa500; }
.lead-cold { border-left: 4px solid #4b8bff; }
</style>
...
st.markdown("# 🎯 ScopeOS")
st.caption("Automation-first B2B Lead OS — alle Quellen, ein Ort")
st.divider()

k1, k2, k3, k4, k5, k6, k7 = st.columns(7)
k1.metric("📋 Leads gesamt", leads_count)
k2.metric("🔥 Hot", tier_counts.get('Hot', 0))
k3.metric("🌤️ Warm", tier_counts.get('Warm', 0))
k4.metric("❄️ Cold", tier_counts.get('Cold', 0))
k5.metric("📈 Heute", today_count)
k6.metric("📆 7 Tage", week_count)
k7.metric("⚡ Aktionen", activities_count)
...
with tab_pipeline:
    ...
    if df_all.empty:
        ...
        if sort_by == 'most recent':
            ...
            df_view = df_view.sort_values(['_tier_ord','created_at'], ascending=[True,False])
            df_view = df_view.drop(columns=['_tier_ord'])
        ...
        for tier in ['Hot','Warm','Cold']:
            ...
            for _, row in tier_df.iterrows():
                with st.container():
                    ...
                    with cols[3]:
                        src_icon = SOURCE_ICON.get(str(row['source']).lower(), '📋')
                        st.caption(f"{src_icon} {row['source']}")
                        st.caption(f"🕐 {freshness_hint(row['created_at'])}")
                    with cols[4]:
                        score_val = row['score']
                        if score_val is not None:
                            color_s = '#ff4b4b' if score_val >= 70 else '#ffa500' if score_val >= 40 else '#4b8bff'
                            st.markdown(f"<div style='text-align:center'><span style='font-size:1.4rem;font-weight:800;color:{color_s}'>{score_val}</span><br><span style='font-size:0.7rem;color:#888'>/100</span></div>", unsafe_allow_html=True)
                    with cols[5]:
                        if row['email'] and row['email'] != '—':
                            st.markdown(f"[📧 Kontakt](mailto:{row['email']}?subject=Follow-up: {row['company']})")
                    st.divider()
...
with tab_analyse:
    ...
    for tier, wf in WORKFLOW_RULES.items():
        ca, cb, cc, cd = st.columns([1,2,2,3])
        ca.markdown(f"**{TIER_EMOJI[tier]} {tier}**")
        cb.caption(f"Owner: {wf['owner']}")
        cc.caption(f"Aktion: {wf['action']}")
        cd.caption(f"Sequenz: {wf['sequence']}")
    st.divider()
...
with tab_quellen:
    ...
    for name, method, path, desc in endpoints:
        ea, eb, ec, ed = st.columns([2,1,3,4])
        ea.markdown(f"**{name}**")
        eb.markdown(f"`{method}`")
        ec.code(path, language=None)
        ed.caption(desc)
    st.divider()
    st.markdown("#### ngrok")
...
with tab_log:
    ...
    if acts:
        ...
        for act in acts:
            icon = {'slack':'💬','crm':'🏢','email':'📧','workflow':'⚡','gmail_scan':'📩','webhook_ingest':'🔗','generic_ingest':'🌐'}.get(act['activity_type'],'📋')
